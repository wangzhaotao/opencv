...
-(void)bilateralFilteImage:(UIImage*)image completion:(void(^)(UIImage *saltNoiseImg, UIImage *filteImg))completion {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        Mat _filteImage;
        cvtColor(mat_img_src,mat_img_src,COLOR_BGRA2BGR); //need 8UC1 or 8UC3
        bilateralFilter(mat_img_src, _filteImage, 25, 25*2, 25/2);
        
        //Mat 转 UIImage
        UIImage *noiseImg = MatToUIImage(salt_image);
        UIImage *filteImg = MatToUIImage(_filteImage);
        
        dispatch_async(dispatch_get_main_queue(), ^{
            if (completion) {
                completion(noiseImg, filteImg);
            }
        });
    });
}
